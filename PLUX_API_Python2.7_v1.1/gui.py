...
def showGraph(frame, graph_btn):
    ...
    graph = make_label(frame, (GetSystemMetrics(0) - 250) / 2 - 300, (GetSystemMetrics(1) - 150) / 2 + 50,
               512, 223, text="Result : ", bg="#ffffff", fg="black", font="Bahnschrift " + str(int(button_ratio * 16)) + " bold", anchor=tk.W,
               image=graph_img)
...
def createHomePage():
    ...
    make_label(main_frame, (GetSystemMetrics(0) - 250) / 2 + ((GetSystemMetrics(0) - 250) * 36 / 100 / 2) + ((GetSystemMetrics(0) - 250) * 3 / 100) + 10, ((GetSystemMetrics(1) - 150) / 2) - ((GetSystemMetrics(1) - 150) * 43 / 100),
               ...
               font="Bahnschrift " + str(int(button_ratio * 18)) , anchor=tk.W)
    ...
    make_label(main_frame, (GetSystemMetrics(0) - 250) / 2 - ((GetSystemMetrics(0) - 250) * 36 / 100 / 2) + ((GetSystemMetrics(0) - 250) * 36 / 100) - ((GetSystemMetrics(0) - 250) / 10) - ((GetSystemMetrics(0) - 250) / 10) + 20, (GetSystemMetrics(1) - 150) / 2 - ((GetSystemMetrics(1) - 150) * 43 / 100) + ((GetSystemMetrics(1) - 150) * 27 / 100) + 10,
                (GetSystemMetrics(0) - 250) / 10, (GetSystemMetrics(1) - 150) * 3 / 100, text='Method', font="Bahnschrift " + str(int(button_ratio * 18)) + " bold", bg='#ffffff', anchor=tk.W)
...
def createAccuracyPage():
    ...
    table_list_knn = [('Class', 'Precision', 'Recall', 'F1-Score'), ('Idle', 0.83, 0.91, 0.87),
              ('Left Click', 0.83, 0.71, 0.77), ('Right Click', 0.78, 0.88, 0.82)]
    total_rows_knn = len(table_list_knn)
    total_columns_knn = len(table_list_knn[0])
    

    table_list_svm = [('Class', 'Precision', 'Recall', 'F1-Score'), ('Idle', 0.77, 0.91, 0.83),
              ('Left Click', 0.85, 0.79, 0.81), ('Right Click', 1.00, 0.88, 0.93)]
    total_rows_svm = len(table_list_svm)
    total_columns_svm = len(table_list_svm[0])
    make_label(main_frame, 10, 10, 200, 30, text="KNN Method", bg="#ffffff", fg="black", 
               font="Bahnschrift 20", anchor=tk.W)
    
    for i in range(total_rows_knn):
        for j in range(total_columns_knn):
            e = tk.Entry(knn_table_frame)
            e.grid(row=i, column=j)
            e.insert(tk.END, table_list_knn[i][j])
    ...
    for i in range(total_rows_svm):
        for j in range(total_columns_svm):
            e = tk.Entry(svm_table_frame)
            e.grid(row=i, column=j)
            e.insert(tk.END, table_list_svm[i][j])
...
def createInformationPage():
    ...
    make_label(main_frame, 10, 140, 870, 300, text=info_file.read(), bg="#ffffff", fg="black", 
               font="Bahnschrift " + str(int(button_ratio * 18)), anchor=tk.CENTER, justify=tk.LEFT)
...
home_button = make_button(window, 0, 100, 200, 50, text=' Home', image=home_icon,
            font="Bahnschrift " + str(int(button_ratio * 18)) + " bold", border=0, bg='white', compound=tk.LEFT,
